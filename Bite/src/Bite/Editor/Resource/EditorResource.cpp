    ...
    EditorResource::EditorResource(EditorSession* session, Rudy::AssetPackage* ownerPackage,const Rudy::AssetType type, const Rudy::String& path,const Rudy::String& name, bool bLoadAsync)
    {
        ...
        */
        ...
        switch (m_AssetType)
        {
            ...
            {
                asset = Rudy::Texture2D::LoadFromDisk(m_AbsolutePath, true, ownerPackage, session->GetApplicationSession()->GetDefaultGraphicsDevice());
                break;
            }
        ...
        * Load asset
        */
        asset->Load(session->GetApplicationSession(),false);
